...
import { subscribeToSession, getStoredUser } from "@/lib/session";

export default function HallOfFamePage() {
  const [tab, setTab] = useState(GAMES[0].id);

  const user = useSyncExternalStore(subscribeToSession, getStoredUser, () => null);

  const rows = useMemo(() => seededScores(tab.length * 23 + 7, 12), [tab]);
  const game = GAMES.find((g) => g.id === tab)!;

  /* Posición y puntuación mock del usuario autenticado */
  const youRank = user ? Math.floor(8 + (tab.length % 4)) : null;
  const youScore = user ? ((rows[5]?.score ?? 10000) - 2400) : null;
  ...
  return (
    <div className="av-hall fade-in">
      ...
      {/* Podio top-3 — orden: plata · oro · bronce */}
      ...
      <div className="hall-table">
        ...
        {/* Fila del usuario autenticado — solo cuando hay sesión */}
